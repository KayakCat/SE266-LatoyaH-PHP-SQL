...
// GET request for editing or adding
if (isGetRequest()) {
    $patientId = filter_input(INPUT_GET, 'id');
    $action = filter_input(INPUT_GET, 'action');

    if ($patientId && $action === 'edit') {
        // Get data if action is edit
...
if (isPostRequest()) {
    ...
    $patientId = filter_input(INPUT_POST, 'patientId');//get patientId hidden input
...
<head>
    ...
    <title><?= ($action === 'edit') ? 'Edit Patient' : 'Add Patient' ?></title>
    <!-- Bootstrap CSS -->
...
<body>
    <div class="container">
    <h2 class="text-center"><?= ($action === 'edit') ? 'Edit Patient' : 'Add Patient' ?></h2>
        ...
        <form name="patients" method="post">
            ...
            <div class="text-center mt-3">
                <input class="btn btn-success" type="submit" name="<?= ($action === 'edit') ? 'updatePatient' : 'storePatient' ?>" value="<?= ($action === 'edit') ? 'Update Patient Information' : 'Add Patient Information' ?>" />
            </div>
        </form>
